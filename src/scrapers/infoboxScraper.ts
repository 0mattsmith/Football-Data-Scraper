 ...
 */
export async function scrapeWikipediaInfobox(title: string): Promise<FootballerProfile | null> {
  ...
  try {
    ...
    const teamsManagedSet = new Set<string>();
    let position: string | undefined;
    let shirtNumber: string | undefined;
    let caps: number | undefined;
    let internationalGoals: number | undefined;
    ...
    const ICONIC_SHIRT_NUMBERS: Record<string, string> = {
      'David Beckham': '#7',
      'Cristiano Ronaldo': '#7',
      'Lionel Messi': '#10',
      'Pelé': '#10',
    ...
    if (clubsArray.some(c => c.includes('Tottenham'))) {
      if (!clubsArray.includes('Tottenham Hotspur')) clubsArray.push('Tottenham Hotspur');
      if (!clubsArray.includes('Spurs')) clubsArray.push('Spurs');
    }
    if (clubsArray.some(c => c.includes('Manchester United') || c.includes('Man Utd'))) {
      if (!clubsArray.includes('Manchester United')) clubsArray.push('Manchester United');
    }

    // 4. Extract Trophies / Honours
    const trophiesSet = new Set<string>();
    ...
    const photoResult = await scrapePlayerPhoto(name, res.data);
