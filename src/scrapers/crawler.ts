 ...
 * European Top Tiers, South America, and NASL/MLS) to expand the Historical Football Knowledge Graph.
 */
...
export async function crawlKnowledgeGraph(category: string = 'efl'): Promise<CrawlResult> {
  ...
  if (target === 'europe' || target === 'all') {
    ...
    const europeStadiums: Record<string, StadiumProfile> = {
  ...
  if (target === 'americas' || target === 'all') {
    const americasClubs: Record<string, ClubProfile> = {
      'boca-juniors': {
    ...
    const americasStadiums: Record<string, StadiumProfile> = {
